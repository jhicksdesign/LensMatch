...
export default function LeadCard({ lead }: { lead: Lead }) {
  ...
  return (
    <div className="group rounded-2xl border border-gallery-border bg-gallery-surface/40 p-6 transition-all duration-400 hover:border-gallery-border-light hover:bg-gallery-surface/60">
      {/* Top row */}
          ...
          <div className="relative inline-flex h-12 w-12 items-center justify-center">
            <svg className="-rotate-90 h-12 w-12" viewBox="0 0 48 48">
              ...
              <circle
                ...
                strokeWidth="2.5"
                strokeLinecap="round"
                strokeDasharray={`${score * 113.1} 113.1`}
      ...
      <div className="mb-4 grid grid-cols-3 gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs font-light capitalize text-gallery-text">
            {lead.eventType}
          </p>
        </div>
        <div>
          ...
          <p className="mt-0.5 text-xs font-light text-gallery-text">
            {lead.eventDate}
          </p>
        </div>
        <div>
          ...
          <p className="mt-0.5 text-xs font-light text-gallery-accent">
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-wrap gap-1.5">
          {lead.selectedStyles.map((s) => (
            <span
              key={s}
              className="rounded-full border border-gallery-border px-2 py-0.5 text-[10px] font-light text-gallery-text-secondary"
            >
              {STYLE_LABELS[s] ?? s}
            </span>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`rounded-full px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider ${status.bg} ${status.text}`}
          >
            {lead.status}
          </span>
        </div>
